...
async function loadDB() {
    try {
        ...
        // database.json tetap menjadi database utama agar fitur lama tidak berubah.
        if (await fs.pathExists(dbFile)) {
            const data = await fs.readFile(dbFile, 'utf8');
            if (data.trim().length > 0) parsed = JSON.parse(data);
        }

        // Fallback: jika database.json belum ada/rusak, pulihkan data user dari users.json.
        if (!parsed && await fs.pathExists(usersFile)) {
            const usersData = await fs.readFile(usersFile, 'utf8');
            if (usersData.trim().length > 0) {
                parsed = { users: JSON.parse(usersData) };
            }
        }
        ...
        if (parsed) {
            ...
            await fs.writeJson(usersFile, db.users, { spaces: 2 });
        } else {
            await saveDB();
        }
        console.log("✅ Database Berhasil Dimuat & users.json tersinkron.");
    } catch (err) {
        console.error("❌ Gagal load database:", err);
        db = { users: {}, scripts: [], redeemCodes: {}, maintenance: false };
    }
}
...
async function saveDB() {
    try {
        ...
        // Salinan khusus user agar ID/info user tetap persisten setelah restart.
        const tmpUsersFile = `${usersFile}.tmp`;
        await fs.writeJson(tmpUsersFile, db.users || {}, { spaces: 2 });
        await fs.move(tmpUsersFile, usersFile, { overwrite: true });
    } catch (err) {
...
// Auto backup saat user baru terdeteksi.
// saveDB() sudah lebih dulu memastikan ID + info user masuk ke users.json.
async function autoBackupNewUser(user) {
    try {
        // Buat backup setelah users.json berisi data user baru.
        const zipPath = await createFullBackup();

        // Kirim ZIP full backup langsung ke Owner.
        await bot.sendDocument(config.ownerId, zipPath, {
            caption: `🆕 <b>BACKUP USER BARU</b>\n\n👤 ID: <code>${escapeHtml(user?.id || '-')}</code>\n📦 Backup: <code>${escapeHtml(path.basename(zipPath))}</code>`,
            parse_mode: 'HTML'
        });

        console.log(`✅ Auto backup new user ${user?.id || '-'} berhasil dibuat & dikirim ke Owner: ${zipPath}`);
    } catch (err) {
        console.error('❌ Gagal auto backup/kirim setelah new user:', err?.message || err);
    }
}
...
// 1. BROADCAST PESAN REPLY (KHUSUS OWNER)
// Cara pakai: reply pesan yang mau dikirim -> ketik /bc
// copyMessage dipakai agar format/entity, foto, video, dokumen, audio, sticker,
// caption, hyperlink, blockquote, dan tipe pesan Telegram yang didukung tetap ikut.
bot.onText(/^\/bc(?:@\w+)?$/, async (msg) => {
    const userId = msg.from.id;
    if (userId !== config.ownerId) return;

    const sourceMessage = msg.reply_to_message;
    if (!sourceMessage || !sourceMessage.message_id) {
        return bot.sendMessage(userId,
            "❌ <b>Cara pakai:</b> reply pesan yang mau di-broadcast, lalu ketik <code>/bc</code>.",
            { parse_mode: 'HTML' }
        );
    }
    ...
    for (const id of userIds) {
        try {
            // Batas aman agar tidak terlalu cepat mengenai rate limit Telegram.
            await new Promise(resolve => setTimeout(resolve, 60));

            // Salin pesan apa adanya; tidak perlu parsing manual berdasarkan tipe media.
            await bot.copyMessage(id, msg.chat.id, sourceMessage.message_id);
            sukses++;
        } catch (err) {
            // Jika Telegram meminta retry karena rate limit, tunggu sesuai retry_after.
            const retryAfter = Number(err?.response?.body?.parameters?.retry_after || 0);
            if (retryAfter > 0) {
            ...
            gagal++;
            console.error(`Broadcast gagal ke ${id}:`, err?.message || err);
        }
    }
...
bot.onText(/\/start (.+)/, async (msg, match) => {
    try {
        ...
        if (!db.users[userId]) {
            ...
            await autoBackupNewUser(msg.from);
        }
...
if (data === "gacha_script") {
    ...
    setTimeout(() => {
        ...
        // Kembalikan ke menu utama sebagai FOTO, sama seperti menu utama awal.
        // Sebelumnya dikirim sebagai text message, sementara handler menu lain
        // memakai editMessageCaption(), sehingga tombol terlihat ada tetapi
        // callback-nya gagal saat mencoba mengedit pesan text tersebut.
        bot.sendPhoto(userId, config.startImage, mainMenu(userId)).catch(() => {
            // Fallback bila startImage gagal: kirim menu text.
            bot.sendMessage(userId, mainMenu(userId).caption, mainMenu(userId)).catch(() => {});
        });
...
// FITUR KLAIM HARIAN
    ...
    if (data === "lucky_spin") {
        ...
        if (data === "owner_menu" && userId === config.ownerId) {
            ...
            [{ text: "♻️ RESET SCRIPT", style: 'success', callback_data: "ask_reset_script" }],
            [{ text: "🔥 RESET DATABASE", style: 'danger', callback_data: "ask_reset" }],
            [{ text: "⬅️ Kembali", style: 'danger', callback_data: "back_home" }]
        ]
                }
            });
...
// --- FITUR RESET SCRIPT SAJA (OWNER ONLY) ---
// Hanya db.scripts yang dikosongkan. Data user, saldo, referral, ban/vip,
// redeem code, maintenance, dan data lain tetap dipertahankan.
if (data === "ask_reset_script" && userId === config.ownerId) {
    const jumlahScript = Array.isArray(db.scripts) ? db.scripts.length : 0;
    return bot.editMessageCaption(
        `<b>⚠️ RESET SCRIPT</b>\n\n` +
        `Aksi ini akan menghapus <b>${jumlahScript}</b> script dari katalog.\n\n` +
        `👥 Data user tetap\n` +
        `💰 Saldo coin tetap\n` +
        `👤 Data akun/referral tetap\n` +
        `🎁 Redeem code tetap\n` +
        `⚙️ Pengaturan bot tetap\n\n` +
        `<i>Yang di-reset hanya daftar script.</i>`,
        {
            chat_id: userId,
            message_id: msgId,
            parse_mode: 'HTML',
            reply_markup: {
                inline_keyboard: [
                    [{ text: "✅ YA, RESET SCRIPT", style: 'success', callback_data: "confirm_reset_script" }],
...
// Eksekusi reset script saja.
if (data === "confirm_reset_script" && userId === config.ownerId) {
    try {
        const jumlahSebelum = Array.isArray(db.scripts) ? db.scripts.length : 0;
        db.scripts = [];
        await saveDB();

        await bot.answerCallbackQuery(query.id, {
            text: `✅ ${jumlahSebelum} script berhasil di-reset!`,
            show_alert: true
        });

        return bot.editMessageCaption(
            `<b>✅ RESET SCRIPT BERHASIL</b>\n\n` +
            `📂 Script sekarang: <b>0</b>\n` +
            `👥 User: <b>${Object.keys(db.users).length}</b> (tetap)\n` +
            `💰 Saldo user tidak diubah.`,
            {
                chat_id: userId,
                message_id: msgId,
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [[{ text: "⬅️ Kembali ke Owner Dashboard", callback_data: "owner_menu" }]]
                }
            }
        );
    } catch (err) {
        console.error("❌ Gagal reset script:", err);
        return bot.answerCallbackQuery(query.id, {
            text: "❌ Gagal reset script. Cek log panel.",
            show_alert: true
        });
    }
}
